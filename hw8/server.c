...
void addMessage(ChatRoom* room,Msg m)
{
    ...
    pthread_cond_broadcast(&room->_condition);
}
...
{
    int nbl = clerk->_room->_nbl;
    Payload p;
    p.code = htonl(PL_SZ);
    p.nb   = htonl(nbl - clerk->_upto);
...
void* handleNewConnection(Clerk* clerk)
{
    ...
    pthread_mutex_unlock(&clerk->_room->_worklock);
    do {
        Command c;
        ...
        switch(c.code){
            ...
            case CC_GET:  // send the named file back to client			
                ...
                pthread_mutex_unlock(&clerk->_room->_loglock);
                break; 
            case CC_LISTEN: // send messages as received to the client
                printf("listen request came in\n");
                listenClient(clerk);
                break;
            case CC_EXIT: 
                done = 1;
